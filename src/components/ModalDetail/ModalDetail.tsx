...
export const ModalDetail = ({ color, position }: ModalDetailProps) => {
  ...
  return (
    <Image src={isGreen ? modalDetailGreen : modalDetailBlue} width={`${isGreen ? 130 : 250}`} height={`${isGreen ? 250 : 130}`} alt="modal detail" className={`absolute hidden laptop:block m-4 modal:m-8 ${svgPositions[position]}`} />
  )
}
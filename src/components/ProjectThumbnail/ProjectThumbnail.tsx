...
export const ProjectThumbnail = ({ project, children }: ProjectThumbnailProps) => {
  ...
  return (
    projectsData.map(({ projectName, thumbImage }, index) => (
      <li key={projectName} className="relative border-t-[3px] last:border-b-[3px] border-white">
        <Link href={`${project === index.toString() ? '?menu=true' : `?project=${index}`}`} className="relative flex items-center modal-lg:h-20 h-full tablet:h-16" >
